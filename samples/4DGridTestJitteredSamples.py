...
if __name__ == "__main__":
	...
	dx =  1.0 / batch_length
	dy =  1.0 / batch_length
	xs = torch.linspace(0, 1-dx, batch_length, device=device)
	ys = torch.linspace(0, 1-dy, batch_length, device=device)
	xn = torch.linspace(0, 1-dx, batch_length, device=device)
	yn = torch.linspace(0, 1-dy, batch_length, device=device)
	nyv, nxv, xv, yv = torch.meshgrid([yn, xn, ys, xs])

	bngrid = torch.stack((nyv.flatten(), nxv.flatten(), yv.flatten(), xv.flatten())).t()
	...
	for i in range(args.n_steps):
		batch = torch.rand([batch_size, 4], device=device, dtype=torch.float32) * dx
		batch = batch + bngrid
